...
public class TownsFlatFileSource extends TownsDataSource {
	...
	public void initialize(Towns plugin, TownsUniverse universe) {
		...
		try {
			FileMgmt.checkFolders(new String[]{rootFolder, rootFolder + dataFolder, rootFolder + dataFolder + FileMgmt.fileSeparator() + "residents", rootFolder + dataFolder + FileMgmt.fileSeparator() + "towns", rootFolder + dataFolder + FileMgmt.fileSeparator() + "towns" + FileMgmt.fileSeparator() + "deleted", rootFolder + dataFolder + FileMgmt.fileSeparator() + "nations", rootFolder + dataFolder + FileMgmt.fileSeparator() + "nations" + FileMgmt.fileSeparator() + "deleted", rootFolder + dataFolder + FileMgmt.fileSeparator() + "worlds", rootFolder + dataFolder + FileMgmt.fileSeparator() + "worlds" + FileMgmt.fileSeparator() + "deleted", rootFolder + dataFolder + FileMgmt.fileSeparator() + "plot-block-data", rootFolder + dataFolder + FileMgmt.fileSeparator() + "townblocks"});
			FileMgmt.checkFiles(new String[]{rootFolder + dataFolder + FileMgmt.fileSeparator() + "residents.txt", rootFolder + dataFolder + FileMgmt.fileSeparator() + "towns.txt", rootFolder + dataFolder + FileMgmt.fileSeparator() + "nations.txt", rootFolder + dataFolder + FileMgmt.fileSeparator() + "worlds.txt", rootFolder + dataFolder + FileMgmt.fileSeparator() + "regen.txt"});
		} catch (IOException e) {
			System.out.println("[Towns] Error: Could not create flatfile default files and folders.");
		}
	}

	@Override
	public void backup() throws IOException {
		String backupType = TownsSettings.getFlatFileBackupType();
		if (! backupType.equalsIgnoreCase("none")) {
			...
			if (backupType.equalsIgnoreCase("folder")) {
				...
				FileMgmt.zipDirectories(new File[]{new File(rootFolder + dataFolder), new File(rootFolder + logFolder), new File(rootFolder + settingsFolder)}, new File(newBackupFolder + ".zip"));
			} else {
	...
	public void cleanupBackups() {
		long deleteAfter = TownsSettings.getBackupLifeLength();
		if (deleteAfter >= 0) {
	...
	public String getWorldFilename(TownsWorld world) {
	...
	public String getPlotFilename(TownBlock townBlock) {
		return rootFolder + dataFolder + FileMgmt.fileSeparator() + "plot-block-data" + FileMgmt.fileSeparator() + townBlock.getWorld().getName() + FileMgmt.fileSeparator() + townBlock.getX() + "_" + townBlock.getZ() + "_" + TownsSettings.getTownBlockSize() + ".data";
	}

	public String getTownBlockFilename(TownBlock townBlock) {
		return rootFolder + dataFolder + FileMgmt.fileSeparator() + "townblocks" + FileMgmt.fileSeparator() + townBlock.getWorld().getName() + FileMgmt.fileSeparator() + townBlock.getX() + "_" + townBlock.getZ() + "_" + TownsSettings.getTownBlockSize() + ".data";
	}
		...
		try {
			while ((line = fin.readLine()) != null) {
				if (! line.equals("")) {
					...
					if (plotData != null) {
						TownsRegenAPI.addPlotChunk(plotData, false);
					}
				}
			}
			fin.close();
			...
			try {
				...
				*/
				...
				if (line != null) {
					...
					if (tokens.length == 3) {
						try {
							TownsWorld world = TownsUniverse.getWorld(tokens[0]);
							...
							try {
								...
								System.out.println("[Towns] [Warning] " + town.getName() + " homeBlock tried to load invalid location.");
							} catch (NotRegisteredException e) {
								System.out.println("[Towns] [Warning] " + town.getName() + " homeBlock tried to load invalid TownBlock.");
							} catch (TownsException e) {
								System.out.println("[Towns] [Warning] " + town.getName() + " does not have a home block.");
							}
						} catch (NotRegisteredException e) {
							System.out.println("[Towns] [Warning] " + town.getName() + " homeBlock tried to load invalid world.");
						}
					}
				}
				...
				if (line != null) {
					...
					if (tokens.length >= 4) {
						try {
							...
							System.out.println("[Towns] [Warning] " + town.getName() + " does not have a spawn point.");
						}
					}
				}
			} catch (Exception e) {
	...
	public boolean loadWorld(TownsWorld world) {
		...
		try {
			...
			System.out.println("[Towns] Loading Error: Exception while reading file " + path);
			e1.printStackTrace();
		}
		...
		if (fileWorld.exists() && fileWorld.isFile()) {
			try {
				...
				line = kvFile.get("usingTowns");
				if (line != null) {
					try {
						world.setUsingTowns(Boolean.parseBoolean(line));
					} catch (Exception e) {
					}
				}
				...
				System.out.println("[Towns] Loading Error: Exception while reading world file " + path);
				e.printStackTrace();
				return false;
			}
			...
			System.out.println("[Towns] Loading Error: File error while reading " + world.getName());
			return false;
		}
	}
		...
		for (TownBlock townBlock : universe.getAllTownBlocks()) {
			...
			if (fileTownBlock.exists() && fileTownBlock.isFile()) {
				try {
					...
					System.out.println("[Towns] Loading Error: Exception while reading TownBlock file " + path);
					e.printStackTrace();
					return false;
				}
				if (! set) {
	...
	public boolean saveResidentList() {
		try {
			...
			System.out.println("[Towns] Saving Error: Exception while saving residents list file");
			e.printStackTrace();
			return false;
		}
	}
	...
	public boolean saveTownList() {
		try {
			...
			System.out.println("[Towns] Saving Error: Exception while saving town list file");
			e.printStackTrace();
			return false;
		}
	}
	...
	public boolean saveNationList() {
		try {
			...
			System.out.println("[Towns] Saving Error: Exception while saving nation list file");
			e.printStackTrace();
			return false;
		}
	}
	...
	public boolean saveWorldList() {
		try {
			System.out.print("[Towns] saveWorldList");
			...
			System.out.println("[Towns] Saving Error: Exception while saving world list file");
			e.printStackTrace();
			return false;
		}
	}
	...
	public boolean saveRegenList() {
		try {
			//System.out.print("[Towns] save active regen list");

			BufferedWriter fout = new BufferedWriter(new FileWriter(rootFolder + dataFolder + FileMgmt.fileSeparator() + "regen.txt"));
			for (PlotBlockData plot : new ArrayList<PlotBlockData>(TownsRegenAPI.getPlotChunks().values())) {
				fout.write(plot.getWorldName() + "," + plot.getX() + "," + plot.getZ() + newLine);
			}
			fout.close();
			return true;
		} catch (Exception e) {
			System.out.println("[Towns] Saving Error: Exception while saving regen file");
			e.printStackTrace();
			return false;
		}
	}
	...
	public boolean saveWorld(TownsWorld world) {
		try {
			...
			// Using Towns
			fout.write("usingTowns=" + Boolean.toString(world.isUsingTowns()) + newLine);
	...
	/*
	 * public boolean saveTownBlocks(TownsWorld world) { try { BufferedWriter
	 ...
	 * System.out.println("[Towns] Loading Error: Exception while saving town blocks list file");
	 * e.printStackTrace();
	 * return false; } }
	 */
	...
	public void utilLoadTownBlocks(String line, Town town, Resident resident) {
		...
		for (String w : worlds) {
			...
			if (split.length != 2) {
				System.out.println("[Towns] [Warning] " + town.getName() + " BlockList does not have a World or data.");
				continue;
			}
			try {
				TownsWorld world = TownsUniverse.getWorld(split[0]);
	...
	public String utilSaveTownBlocks(List<TownBlock> townBlocks) {
		HashMap<TownsWorld, ArrayList<TownBlock>> worlds = new HashMap<TownsWorld, ArrayList<TownBlock>>();
		String out = "";
		...
		for (TownsWorld world : worlds.keySet()) {
			out += world.getName() + ":";
	...
	public void deleteTown(Town town) {
		...
		if (file.exists()) {
			try {
				...
				System.out.println("[Towns] Error moving Town txt file.");
			}
		}
	}
	...
	public void deleteNation(Nation nation) {
		...
		if (file.exists()) {
			try {
				...
				System.out.println("[Towns] Error moving Nation txt file.");
			}
		}
	}

	@Override
	public void deleteWorld(TownsWorld world) {
		File file = new File(getWorldFilename(world));
		if (file.exists()) {
			try {
				FileMgmt.moveFile(file, ("deleted"));
			} catch (IOException e) {
				System.out.println("[Towns] Error moving World txt file.");
			}
		}
	}
