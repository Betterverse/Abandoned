...
import net.betterverse.towns.config.CommentedConfiguration;
...
import net.betterverse.towns.object.TownsPermission.PermLevel;
...
public class TownsSettings {
	...
	private static final SortedMap<Integer, Map<TownsSettings.TownLevel, Object>> configTownLevel = Collections.synchronizedSortedMap(new TreeMap<Integer, Map<TownsSettings.TownLevel, Object>>(Collections.reverseOrder()));
	private static final SortedMap<Integer, Map<TownsSettings.NationLevel, Object>> configNationLevel = Collections.synchronizedSortedMap(new TreeMap<Integer, Map<TownsSettings.NationLevel, Object>>(Collections.reverseOrder()));

	private static final Map<String, Map<TownsSettings.ChatChannel, String>> configChatChannel = Collections.synchronizedSortedMap(new TreeMap<String, Map<TownsSettings.ChatChannel, String>>(Collections.reverseOrder()));

	private static final Set<Material> bypassZoneMaterials = EnumSet.noneOf(Material.class);
	...
	public static void newTownLevel(int numResidents, String namePrefix, String namePostfix, String mayorPrefix, String mayorPostfix, int townBlockLimit, double townUpkeepMultiplier) {
		ConcurrentHashMap<TownsSettings.TownLevel, Object> m = new ConcurrentHashMap<TownsSettings.TownLevel, Object>();
		m.put(TownsSettings.TownLevel.NAME_PREFIX, namePrefix);
		m.put(TownsSettings.TownLevel.NAME_POSTFIX, namePostfix);
		m.put(TownsSettings.TownLevel.MAYOR_PREFIX, mayorPrefix);
		m.put(TownsSettings.TownLevel.MAYOR_POSTFIX, mayorPostfix);
		m.put(TownsSettings.TownLevel.TOWN_BLOCK_LIMIT, townBlockLimit);
		m.put(TownsSettings.TownLevel.UPKEEP_MULTIPLIER, townUpkeepMultiplier);
		configTownLevel.put(numResidents, m);
	}

	public static void newNationLevel(int numResidents, String namePrefix, String namePostfix, String capitalPrefix, String capitalPostfix, String kingPrefix, String kingPostfix, double nationUpkeepMultiplier) {
		ConcurrentHashMap<TownsSettings.NationLevel, Object> m = new ConcurrentHashMap<TownsSettings.NationLevel, Object>();
		m.put(TownsSettings.NationLevel.NAME_PREFIX, namePrefix);
		m.put(TownsSettings.NationLevel.NAME_POSTFIX, namePostfix);
		m.put(TownsSettings.NationLevel.CAPITAL_PREFIX, capitalPrefix);
		m.put(TownsSettings.NationLevel.CAPITAL_POSTFIX, capitalPostfix);
		m.put(TownsSettings.NationLevel.KING_PREFIX, kingPrefix);
		m.put(TownsSettings.NationLevel.KING_POSTFIX, kingPostfix);
		m.put(TownsSettings.NationLevel.UPKEEP_MULTIPLIER, nationUpkeepMultiplier);
	...
	public static void newChatChannel(String command, String name, String colour, String node) {
		Map<TownsSettings.ChatChannel, String> push = new ConcurrentHashMap<TownsSettings.ChatChannel, String>();

		push.put(TownsSettings.ChatChannel.COMMAND, command.toLowerCase());
		push.put(TownsSettings.ChatChannel.CHANNEL_NAME, name);
		push.put(TownsSettings.ChatChannel.TEXT_COLOUR, colour);
		push.put(TownsSettings.ChatChannel.PERMISSION_NODE, node);
	...
	public static void loadNationLevelConfig() throws IOException {
		...
		for (Map<?, ?> level : levels) {
			newNationLevel((Integer) level.get("numResidents"), (String) level.get("namePrefix"), (String) level.get("namePostfix"), (String) level.get("capitalPrefix"), (String) level.get("capitalPostfix"), (String) level.get("kingPrefix"), (String) level.get("kingPostfix"), (Double) level.get("upkeepModifier"));
		}
	...
	public static void loadBypassMaterials() throws IOException {
		List<Integer> ids = getIntArr(ConfigNodes.GTOWN_SETTINGS_BYPASS_BLOCKS);
		for (Integer id : ids) {
			Material mat = Material.getMaterial(id);
			if (mat != null) {
				bypassZoneMaterials.add(mat);
			}
		}
	}

	public static void loadChatChannelsConfig() throws IOException {
		List<String> chatChannels = config.getStringList(ConfigNodes.FILTERS_MODIFY_CHAT_CHANNELS.getRoot());
	...
	public static Map<TownsSettings.TownLevel, Object> getTownLevel(int numResidents) {
		return configTownLevel.get(numResidents);
	}

	public static Map<TownsSettings.NationLevel, Object> getNationLevel(int numResidents) {
		return configNationLevel.get(numResidents);
	}

	public static Map<TownsSettings.TownLevel, Object> getTownLevel(Town town) {
		return getTownLevel(calcTownLevel(town));
	}

	public static Map<TownsSettings.NationLevel, Object> getNationLevel(Nation nation) {
	...
	public static void loadCachedObjects() throws IOException {
		...
		TownsWarConfig.setFlagBaseMaterial(Material.matchMaterial(getString(ConfigNodes.WAR_ENEMY_FLAG_BASE_BLOCK)));
		TownsWarConfig.setFlagLightMaterial(Material.matchMaterial(getString(ConfigNodes.WAR_ENEMY_FLAG_LIGHT_BLOCK)));
		TownsWarConfig.setBeaconWireFrameMaterial(Material.matchMaterial(getString(ConfigNodes.WAR_ENEMY_BEACON_WIREFRAME_BLOCK)));
		...
		TownsWarConfig.setEditableMaterialsInWarZone(getAllowedMaterials(ConfigNodes.WAR_WARZONE_EDITABLE_MATERIALS));
	...
	private static void sendError(String msg) {
		System.out.println("[Towns] Error could not read " + msg);
	}
	...
	*/
	...
	private static void setChannels() {
		...
		if (chatChannels == null || chatChannels.isEmpty() || chatChannels.size() == 0) {
			...
			newChannels.add("/tc,&f[&3TC&f],&b,towns.chat.town");
			newChannels.add("/nc,&f[&6NC&f],&e,towns.chat.nation");
			newChannels.add("/a,&f[&4ADMIN&f],&c,towns.chat.admin");
			newChannels.add("/m,&f[&9MOD&f],&5,towns.chat.mod");
	...
	private static void setDefaultLevels() {
		...
		List<?> townLevels = config.getList(ConfigNodes.LEVELS_TOWN_LEVEL.getRoot());
		...
		List<?> nationLevels = config.getList(ConfigNodes.LEVELS_NATION_LEVEL.getRoot());
	...
	public static String[] getJoinWarMsg(TownsObject obj) {
	...
	public static String[] getCouldntPayTaxesMsg(TownsObject obj, String type) {
	...
	public static String getKingPrefix(Resident resident) {
		try {
			return (String) getNationLevel(resident.getTown().getNation()).get(TownsSettings.NationLevel.KING_PREFIX);
	...
	public static String getMayorPrefix(Resident resident) {
		try {
			return (String) getTownLevel(resident.getTown()).get(TownsSettings.TownLevel.MAYOR_PREFIX);
		} catch (NotRegisteredException e) {
	...
	public static String getCapitalPostfix(Town town) {
		try {
			return (String) getNationLevel(town.getNation()).get(TownsSettings.NationLevel.CAPITAL_POSTFIX);
		} catch (NotRegisteredException e) {
	...
	public static String getTownPostfix(Town town) {
		try {
			return (String) getTownLevel(town).get(TownsSettings.TownLevel.NAME_POSTFIX);
		} catch (Exception e) {
	...
	public static String getNationPostfix(Nation nation) {
		try {
			return (String) getNationLevel(nation).get(TownsSettings.NationLevel.NAME_POSTFIX);
		} catch (Exception e) {
	...
	public static String getNationPrefix(Nation nation) {
		try {
			return (String) getNationLevel(nation).get(TownsSettings.NationLevel.NAME_PREFIX);
		} catch (Exception e) {
	...
	public static String getTownPrefix(Town town) {
		try {
			return (String) getTownLevel(town).get(TownsSettings.TownLevel.NAME_PREFIX);
		} catch (Exception e) {
	...
	public static String getCapitalPrefix(Town town) {
		try {
			return (String) getNationLevel(town.getNation()).get(TownsSettings.NationLevel.CAPITAL_PREFIX);
		} catch (NotRegisteredException e) {
	...
	public static String getKingPostfix(Resident resident) {
		try {
			return (String) getNationLevel(resident.getTown().getNation()).get(TownsSettings.NationLevel.KING_POSTFIX);
		} catch (NotRegisteredException e) {
	...
	public static String getMayorPostfix(Resident resident) {
		try {
			return (String) getTownLevel(resident.getTown()).get(TownsSettings.TownLevel.MAYOR_POSTFIX);
		} catch (NotRegisteredException e) {
	...
	public static int getMaxTownBlocks(Town town) {
		...
		if (ratio == 0) {
			n += (Integer) getTownLevel(town).get(TownsSettings.TownLevel.TOWN_BLOCK_LIMIT);
		} else {
	...
	*/
	...
	public static List<String> getWorldMobRemovalEntities() {
		if (getDebug()) {
			System.out.println("[Towns] Debug: Reading World Mob removal entities. ");
		}
	...
	public static List<String> getTownMobRemovalEntities() {
		if (getDebug()) {
			System.out.println("[Towns] Debug: Reading Town Mob removal entities. ");
		}
	...
	public static boolean isBypassMaterial(Material mat) {
		return bypassZoneMaterials.contains(mat);
	}
	...
	public static List<String> getDisallowedTownSpawnZones() {
		if (getDebug()) {
			System.out.println("[Towns] Debug: Reading disallowed town spawn zones. ");
		}
	...
	public static double getTownUpkeepCost(Town town) {
		...
		if (town != null) {
			if (isUpkeepByPlot()) {
				...
				multiplier = Double.valueOf(getTownLevel(town).get(TownsSettings.TownLevel.UPKEEP_MULTIPLIER).toString());
			}
		} else {
			multiplier = 1.0;
		}
	...
	public static double getNationUpkeepCost(Nation nation) {
		...
		if (nation != null) {
			multiplier = Double.valueOf(getNationLevel(nation).get(TownsSettings.NationLevel.UPKEEP_MULTIPLIER).toString());
		} else {
			multiplier = 1.0;
		}
	...
	public static long getBackupLifeLength() {
		long t = TimeTools.getMillis(TownsSettings.getString(ConfigNodes.PLUGIN_BACKUPS_ARE_DELETED_AFTER));
	...
	public static boolean isTownsUpdating(String currentVersion) {
		if (isTownsUpToDate(currentVersion)) {
			return false;
		} else {
			return true; //Assume
		}
	...
	public static boolean isTownsUpToDate(String currentVersion) {
	...
	public static int getMaxResidentPlots(Resident resident) {
		int maxPlots = TownsUniverse.getPermissionSource().getGroupPermissionIntNode(resident.getName(), "towns_maxplots");
		if (maxPlots == - 1) {
	...
	public static boolean isValidRegionName(String name) {
		if ((name.toLowerCase() == "spawn") || (name.equalsIgnoreCase("list")) || (name.equalsIgnoreCase("new")) || (name.equalsIgnoreCase("here")) || (name.equalsIgnoreCase("new")) || (name.equalsIgnoreCase("help")) || (name.equalsIgnoreCase("?")) || (name.equalsIgnoreCase("leave")) || (name.equalsIgnoreCase("withdraw")) || (name.equalsIgnoreCase("deposit")) || (name.equalsIgnoreCase("set")) || (name.equalsIgnoreCase("toggle")) || (name.equalsIgnoreCase("mayor")) || (name.equalsIgnoreCase("assistant")) || (name.equalsIgnoreCase("kick")) || (name.equalsIgnoreCase("add")) || (name.equalsIgnoreCase("claim")) || (name.equalsIgnoreCase("unclaim")) || (name.equalsIgnoreCase("title"))) {
			return false;
		}
	...
	public static boolean isValidName(String name) {
		try {
			if (TownsSettings.namePattern == null) {
				namePattern = Pattern.compile(getNameCheckRegex());
			}
			return TownsSettings.namePattern.matcher(name).find();
