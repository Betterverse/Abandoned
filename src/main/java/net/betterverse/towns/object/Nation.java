...
public class Nation extends TownsEconomyObject implements ResidentList {
	...
	public void setNeutral(boolean neutral) throws TownsException {
		if (! TownsSettings.isDeclaringNeutral() && neutral) {
			throw new TownsException(TownsSettings.getLangString("msg_err_fight_like_king"));
		} else {
			if (neutral) {
				for (Resident resident : getResidents()) {
					TownsWar.removeAttackerFlags(resident.getName());
				}
			}
	...
	public void setKing(Resident king) throws TownsException {
		if (! hasResident(king)) {
			throw new TownsException(TownsSettings.getLangString("msg_err_king_not_in_nation"));
		}
		if (! king.isMayor()) {
			throw new TownsException(TownsSettings.getLangString("msg_err_new_king_notmayor"));
		}
	...
	public void withdrawFromBank(Resident resident, int amount) throws TownsException {
		if (! isKing(resident) && ! hasAssistant(resident)) {
			throw new TownsException(TownsSettings.getLangString("msg_no_access_nation_bank"));
		}

		if (TownsSettings.isUsingEconomy()) {
			if (! payTo(amount, resident, "Nation Withdraw")) {
				throw new TownsException(TownsSettings.getLangString("msg_err_no_money"));
			}
		} else {
			throw new TownsException(TownsSettings.getLangString("msg_err_no_economy"));
		}
	}
