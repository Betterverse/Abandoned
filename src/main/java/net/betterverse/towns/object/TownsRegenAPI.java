...
public class TownsRegenAPI extends TownsUniverse {
	...
	public static void deletePlotChunk(PlotBlockData plotChunk) {
		if (PlotChunks.containsKey(getPlotKey(plotChunk))) {
			...
			TownsUniverse.getDataSource().saveRegenList();
		}
	}
	...
	public static void addPlotChunkSnapshot(PlotBlockData plotChunk) {
		if (TownsUniverse.getDataSource().loadPlotData(plotChunk.getWorldName(), plotChunk.getX(), plotChunk.getZ()) == null) {
			TownsUniverse.getDataSource().savePlotData(plotChunk);
		}
	}
	...
	public static void deletePlotChunkSnapshot(PlotBlockData plotChunk) {
		TownsUniverse.getDataSource().deletePlotData(plotChunk);
	}
	...
	public static PlotBlockData getPlotChunkSnapshot(TownBlock townBlock) {
		return TownsUniverse.getDataSource().loadPlotData(townBlock);
	}
