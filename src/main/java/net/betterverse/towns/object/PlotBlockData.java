...
public class PlotBlockData {
	...
	public PlotBlockData(TownBlock townBlock) {
		setX(townBlock.getX());
		setZ(townBlock.getZ());
		setSize(TownsSettings.getTownBlockSize());
		this.worldName = townBlock.getWorld().getName();
		this.setVersion(defaultVersion);
		try {
			setHeight(TownsUniverse.plugin.getServerWorld(worldName).getMaxHeight() - 1);
	...
	private List<Integer> getBlockArr() {
		...
		try {
			World world = TownsUniverse.plugin.getServerWorld(worldName);
			/*
		...
		try {
			...
			while (reverse > 0) {
				...
				if ((blockId != storedData.getTypeID())) {
					if (! TownsUniverse.getWorld(worldName).isPlotManagementIgnoreIds(storedData.getTypeID())) {
