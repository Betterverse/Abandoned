...
public class CellUnderAttack extends Cell {
	...
	public void loadBeacon() {
		...
		if (! TownsWarConfig.isDrawingBeacon()) {
			return;
		}

		int beaconSize = TownsWarConfig.getBeaconSize();
	...
	private Block getBeaconMinBlock(World world) {
		...
		int radiusCenterExpansion = TownsWarConfig.getBeaconRadius() - 1;
		int fromCorner = middle - radiusCenterExpansion;
		int maxY = world.getMaxHeight();

		int x = (getX() * Coord.getCellSize()) + fromCorner;
		int y = maxY - TownsWarConfig.getBeaconSize();
	...
	public boolean hasEnded() {
		return flagColorId >= TownsWarConfig.getWoolColors().length;
	}
	...
	public void drawFlag() {
		...
		flagBaseBlock.setType(TownsWarConfig.getFlagBaseMaterial());
		updateFlag();
		flagLightBlock.setType(TownsWarConfig.getFlagLightMaterial());
		for (Block block : beaconWireframeBlocks) {
			block.setType(TownsWarConfig.getBeaconWireFrameMaterial());
		}
	}

	public void updateFlag() {
		DyeColor[] woolColors = TownsWarConfig.getWoolColors();
